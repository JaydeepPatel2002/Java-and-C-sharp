...
public class Exercise
{
	...
	public static void main(String[] args)
	{
		...
		Map<Trader,List<Transaction>> transByYear = transactions.stream()
						.collect(Collectors.groupingBy(t->t.getTrader()));
						
		transactions.stream()
		.collect(Collectors.averagingInt(Transaction::getValue));
		
		Map<Integer,Map<String, List<Transaction>>> foo = transactions.stream()
						.collect(
		...
		Map<Trader,List<Transaction>> transByTrader = transactions.stream()
				.collect(Collectors.groupingBy(t->t.getTrader()));

		Map<Trader, Long> countsByTrader = transactions.stream()
				.collect(Collectors.groupingBy(t->t.getTrader(),Collectors.counting()));
		
		Map<Trader, Optional<Transaction>> maxByTrader = transactions.stream()
				.collect(Collectors.groupingBy(t->t.getTrader(),
